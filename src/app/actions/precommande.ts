...
export async function soumettrePrecommande(formData: FormData) {
  ...
  if (process.env.RESEND_API_KEY) {
    await resend.emails.send({
      from: 'YES BOX <noreply@yesbox-lepacte.fr>',
          ...
          <div style="padding: 32px; color: #1a1816;">
            <table style="width: 100%; border-collapse: collapse; font-size: 14px;">
              <tr style="border-bottom: 1px solid #e6dfd1;">
                <td style="padding: 10px 0; color: #736c63; width: 120px;">Prénom</td>
                <td style="padding: 10px 0; font-weight: 600;">${prenom}</td>
              </tr>
              <tr style="border-bottom: 1px solid #e6dfd1;">
                <td style="padding: 10px 0; color: #736c63;">Email</td>
                <td style="padding: 10px 0;"><a href="mailto:${email}" style="color: #c5256e;">${email}</a></td>
              </tr>
            ...
            <p style="margin: 24px 0 0; font-size: 12px; color: #736c63;">Reçu le ${new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
          </div>
        </div>
      `,
  ...
  // Email de confirmation au pré-commandeur
  if (process.env.RESEND_API_KEY) {
    await resend.emails.send({
      from: 'YES BOX <noreply@yesbox-lepacte.fr>',
      to: email,
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 520px; margin: 0 auto; background: #fbf8f3; border-radius: 16px; overflow: hidden;">
          ...
          <div style="padding: 32px; color: #1a1816;">
            ...
            <p style="font-size: 15px; line-height: 1.7; color: #3b3733; margin: 0 0 16px;">
              Merci pour ta pré-commande ! Tu fais partie des premiers couples à rejoindre <strong>YES BOX — Le Pacte</strong>.
            </p>
            <p style="font-size: 15px; line-height: 1.7; color: #3b3733; margin: 0 0 24px;">
              Le programme ouvre le <strong>1er septembre 2026</strong>. Tu recevras un email dès que tu pourras accéder à ton espace couple et commencer le Module 1 gratuitement.
            </p>
            ...
            <p style="font-size: 13px; color: #736c63; margin: 0;">
              Des questions ? Réponds à cet email ou écris-nous à <a href="mailto:contact@yesbox-lepacte.fr" style="color: #c5256e;">contact@yesbox-lepacte.fr</a>.
            </p>
          </div>
